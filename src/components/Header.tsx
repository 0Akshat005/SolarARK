...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 pointer-events-none transition-all duration-300">
      <div className="max-w-[1440px] mx-auto px-3 sm:px-5 lg:px-6 pt-2 sm:pt-2.5">
        
        {/* Floating Luxury Glass Capsule Navigation Bar (Sweet Spot: Translucent, Unobstructed, Highly Readable) */}
        <div
          className={`pointer-events-auto flex items-center justify-between gap-3 sm:gap-4 px-4 sm:px-6 py-2 sm:py-2.5 rounded-full transition-all duration-300 ${
            isDarkHero
              ? 'bg-[#0A0F1D]/50 backdrop-blur-xl border border-white/20 shadow-[0_12px_36px_rgba(0,0,0,0.35),_inset_0_1px_1px_rgba(255,255,255,0.15)]'
              : isScrolled
              ? 'bg-white/92 backdrop-blur-xl border border-stone-200/90 shadow-[0_6px_24px_rgba(0,0,0,0.06)]'
              : 'bg-white/90 backdrop-blur-xl border border-stone-200/80 shadow-[0_4px_20px_rgba(0,0,0,0.05)]'
          }`}
        >
          {/* 1. LEFT: Brand Identity & Tagline */}
          ...
          {/* 2. CENTER: Symmetrical Navigation Rail */}
          <nav 
          ...
          {/* 3. RIGHT: Action Cluster (Urgency Call Icon + High-Conversion CTA) */}
      ...
      {/* Mobile Drawer Menu */}
      {mobileMenuOpen && (
        <div className="pointer-events-auto max-w-[1440px] mx-auto px-3 sm:px-5 lg:px-6 pt-2">
          <div className="bg-white/95 backdrop-blur-2xl border border-stone-200/90 rounded-3xl p-5 space-y-3 shadow-2xl animate-in slide-in-from-top-2 duration-200">
